...
<head>
    ...
    <style>
        .form-card{max-width:680px;margin:2rem auto;background:#fff;border:1px solid #e5e7eb;border-radius:.75rem;padding:1rem}
        .row{display:flex;gap:.75rem;flex-wrap:wrap}
        .row > div{flex:1;min-width:240px}
        .field-error{ color:#b91c1c; font-size:.9rem; margin-top:.25rem }
        .input-error{ border-color:#fecaca; background:#fff7f7 }
        .flash{background:#ecfdf5;border:1px solid #86efac;padding:.6rem .8rem;border-radius:.5rem;color:#166534;margin:1rem auto;max-width:680px}
    </style>
    <script>
    ...
    <form action="<c:url value='/demo/submit'/>" method="post" novalidate>
        <div class="row">
            <div>
                ...
                <c:if test="${errors['email'] != null}"><div class="field-error">${errors['email']}</div></c:if>
            </div>
            <div>
                <label>Âge</label>
                ...
                <c:if test="${errors['age'] != null}"><div class="field-error">${errors['age']}</div></c:if>
            </div>
        </div>
        ...
        <div class="row">
            <div>
                ...
                <c:if test="${errors['password'] != null}"><div class="field-error">${errors['password']}</div></c:if>
            </div>
            <div>
                <label>Confirmation</label>
                <input type="password" name="confirm" required
                       class="${errors['confirm'] != null ? 'input-error' : ''}">
                <c:if test="${errors['confirm'] != null}"><div class="field-error">${errors['confirm']}</div></c:if>
            </div>
        </div>
        ...
        <div class="row">
            <div>
                ...
                <c:if test="${errors['website'] != null}"><div class="field-error">${errors['website']}</div></c:if>
            </div>
            <div>
                ...
                <c:if test="${errors['phone'] != null}"><div class="field-error">${errors['phone']}</div></c:if>
            </div>
        </div>
        ...
        <div class="row">
            <div>
                ...
                <c:if test="${errors['date'] != null}"><div class="field-error">${errors['date']}</div></c:if>
            </div>
        </div>
